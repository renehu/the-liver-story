...
const TARGET_SUB_CHAPTER_ID = 25;
const TARGET_IDENTIFIER = "damaged_liver";
const OUTPUT_DIR = "damaged_liver_mp4";
...
async function main() {
  ...
  for (const jsonFile of jsonFiles) {
    ...
    for (const chapter of json.data.list || []) {
      for (const sub of chapter.sub_chapters || []) {
        if (
          sub.sub_chapter_id !== TARGET_SUB_CHAPTER_ID ||
          sub.identifier !== TARGET_IDENTIFIER
        ) {
          continue;
        }
  ...
  fs.writeFileSync(
    "download-damaged-liver-failed.json",
  ...
  console.log("Failed list saved to download-damaged-liver-failed.json");
}
